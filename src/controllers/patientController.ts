...
export const getPatients = async (req: Request, res: Response) => {
  ...
  try {
    if (address) {
      const patients = await dynamoDBService.findPatientsByAddress(address as string);
       res.status(201).json(patients);
    }else if (condition) {
      const patients = await openSearchService.searchPatientsByCondition(condition as string);
       res.status(201).json(patients);
    } else {
      res.status(400).json({ error: 'Provide a valid query address or condition' });
    } 
  } catch (err) {
    logger.error('error',{stack:err})
...
export const updatePatient = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updatedPatient = await dynamoDBService.updatePatient(id, { name, address, conditions, allergies });
    ...
    if(checkPatient.length){
      const openSearchID = checkPatient[0]._id
       openSearchCheckForUpdate =  await openSearchService.updatePatientIndex(openSearchID, updatedPatient);
       if(openSearchCheckForUpdate && openSearchCheckForUpdate.body.result == 'updated'){
        openSearchData = await openSearchService.searchPatientsByPatientId(id)
       }else {
        openSearchData = openSearchCheckForUpdate
       }
    }   
    res.status(201).json({updatedPatient,openSearch:openSearchData});
  } catch (err) {
    logger.error('error',{stack:err})
...
export const searchPatientsByCondition = async (req: Request, res: Response) => {
